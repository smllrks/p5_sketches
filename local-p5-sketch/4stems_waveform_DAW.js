...
function draw() {
  ...
  let waveformHeight = height / numStems;
  
  for (let i = 0; i < numStems; i++) {
    // Draw center line for each waveform
    stroke(50);
    let centerY = i * waveformHeight + waveformHeight / 2;
    ...
    if (stems[i].isPlaying()) {
      ...
      stroke(255);
      line(playbackPosition, i * waveformHeight, playbackPosition, (i + 1) * waveformHeight);
    ...
    textAlign(LEFT, TOP);
    textSize(16);
    text(`Stem ${i+1}`, 10, i * waveformHeight + 10);
  }
}

function drawWaveform(col, endX, stemIndex, waveformHeight) {
  let centerY = stemIndex * waveformHeight + waveformHeight / 2;
  ...
  for (let i = 0; i < endX; i++) {
    let x = i;
    let y = map(abs(waveforms[stemIndex][i]), 0, 1, centerY, centerY - waveformHeight/1);  // Increased amplitude
    vertex(x, y);
  }
  vertex(endX, centerY);
  endShape();
